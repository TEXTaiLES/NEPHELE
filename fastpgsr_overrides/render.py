...
#
# This is upstream FastGS's own render.py with a SINGLE bug fix: render_set()
# used an undefined `mesh_path` when writing mesh_color.ply / mesh_nocolor.ply,
# crashing every mesh export with NameError before any .ply was written (which
# also blocked render_sets() from writing tsdf_fusion.ply). See the "PROJECT
# FIX" comment inside render_set(). FastGS's accelerated rendering/TSDF logic is
# otherwise unchanged.
# ---------------------------------------------------------------------------
...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background, args,
    ...
    if volume is not None:
        # PROJECT FIX (Nephele / fastpgsr): upstream FastGS render.py references
        # `mesh_path` below (mesh_color.ply / mesh_nocolor.ply) without ever
        # defining it, so mesh export crashes with NameError on every run. Define
        # it here to the same `<model_path>/mesh` dir that render_sets() uses for
        # tsdf_fusion.ply, so both write to one place. This is the only change vs
        # upstream render.py — the accelerated rendering logic is untouched.
        mesh_path = os.path.join(model_path, "mesh")
