...
class Camera(nn.Module):
    ...
    def get_image(self):
        if self.preload_img:
            m = self.mask.cuda() if self.mask is not None else None
            return self.original_image.cuda(), self.original_image_gray.cuda(), m
        else:
            gt_image, gray_image, loaded_mask = process_image(self.image_path, self.resolution, self.ncc_scale)
            m = loaded_mask.cuda() if loaded_mask is not None else None
            return gt_image.cuda(), gray_image.cuda(), m
