...
def post_process_mesh(mesh, cluster_to_keep=1):
    ...
    if len(cluster_n_triangles) == 0:
        print("[!] TSDF fusion produced an empty mesh (0 triangles). "
              "This usually means COLMAP camera poses are too sparse or inaccurate "
              "for the object. Try full quality mode (PIPELINE_MODE=full) or "
              "capture more images with better lighting.")
        return mesh_0
...
def render_set(model_path, name, iteration, views, scene, gaussians, pipeline, background,
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        if name == 'test':
            ...
            rendering_np = (rendering.permute(1,2,0).clamp(0,1)*255).detach().cpu().numpy().astype(np.uint8)
            from PIL import Image as _PIL
            if _CV2:
                cv2.imwrite(os.path.join(render_path, view.image_name + ".jpg"), rendering_np[:,:,[2,1,0]])
            else:
                _PIL.fromarray(rendering_np).save(os.path.join(render_path, view.image_name + ".jpg"), format='JPEG')
