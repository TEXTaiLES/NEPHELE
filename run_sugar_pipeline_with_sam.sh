...
# This script lives at the monorepo root; data/logs/outputs go here
SUGAR_PATH="${SUGAR_PATH:-$SCRIPT_DIR}"

# Monorepo root is the same directory as this script
MONO_ROOT="${nephele_PATH:-$SCRIPT_DIR}"

# SuGaR repo scripts (convert_to_rgba.py, cleanup_mesh.py, Dockerfile_final)
SUGAR_REPO_PATH="$SCRIPT_DIR/SUGAR/SuGaR"
...
VOL_SCRIPTS="-v $SUGAR_REPO_PATH:/app/sugar_scripts:ro"
...
DOCKER_COMMON=(
  ...
  $VOL_DATA $VOL_OUT $VOL_CACHE $VOL_SCRIPTS
  "${ENV_COMMON[@]}"
)
...
run_in_sugar() {
  ...
  local REDIRECT_OUT='mkdir -p /app/output && mkdir -p /workspace && rm -rf /workspace/output && ln -sfn /app/output /workspace/output'
...
run_in_sugar "$(cat << 'SUGAR_SCRIPT'
python3 - << 'PREFLIGHT'
import os, glob, shutil, re
from PIL import Image

images_dir = '/app/data/images_sugar'
sparse_dir = '/app/data/distorted/sparse/0'
...
run_in_sugar "python3 /app/sugar_scripts/convert_to_rgba.py /app/data/images /app/data/sparse/0/images.bin 2 ${BLACK_THRESHOLD:-15}"
...
if [[ -n "$MESH_OBJ" ]]; then
  MESH_OBJ_DOCKER="/app/output/${MESH_OBJ#$SUGAR_OUT_ROOT/}"
  run_in_sugar "python3 /app/sugar_scripts/cleanup_mesh.py \"$MESH_OBJ_DOCKER\" 0.05 $BOTTOM_CROP_PCT 2"
else
