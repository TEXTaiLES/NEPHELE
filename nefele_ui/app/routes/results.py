...
def download_one(relative: str):
    ...
    raw = target.name if model == "pgsr" else friendly_name(target.name, c.dataset_name)
    dl_name = raw if raw.startswith(f"{c.dataset_name}_") else f"{c.dataset_name}_{raw}"
...
def download_zip():
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for f in files:
            ...
            raw = src.name if model == "pgsr" else f.name
            # Ensure every file inside the zip carries the dataset prefix.
            arc_name = raw if raw.startswith(f"{c.dataset_name}_") else f"{c.dataset_name}_{raw}"
